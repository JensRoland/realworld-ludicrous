<div class="article-preview">
    <div class="article-meta" style="view-transition-name: <?= htmlspecialchars($slug) ?>-meta;">
        <a fx-yolo-deep href="/profile/<?= htmlspecialchars($authorUsername) ?>" title="Author Profile">
            <img src="<?= htmlspecialchars($authorImageThumb) ?>" width="32" height="32" alt="author avatar" />
        </a>
        <div class="info">
            <a fx-yolo-deep href="/profile/<?= htmlspecialchars($authorUsername) ?>" class="author"><?= htmlspecialchars($authorUsername) ?></a>
            <span class="date"><?= $date ?></span>
        </div>
        <?php \App\Components\FavoriteButton\render($article, $isFavorited, true, true); ?>
    </div>
    <a fx-yolo-deep href="/article/<?= htmlspecialchars($slug) ?>" class="preview-link">
        <h1 style="view-transition-name: <?= htmlspecialchars($slug) ?>-title;">
            <?= htmlspecialchars($title) ?>
        </h1>
        <p><?= htmlspecialchars($description) ?></p>
        <span>Read more...</span>
        <ul class="tag-list">
            <?php foreach ($tagList as $tag): ?>
                <li class="tag-default tag-pill tag-outline"><?= htmlspecialchars($tag) ?></li>
            <?php endforeach; ?>
        </ul>
    </a>
</div>
